...
int main() {
    ...
    G.deck[0][2] = smithy;
    G.deck[0][3] = silver; //treasure #2
    cardEffect(adventurer, -1, -1, -1, &G, 0, -1); //simulate adventurer card play
    printf("Testing for treasures in right most two positions\n");
    ...
    printf("Testing for silver at position 7, expected value:5\n");     
     if (assertTrue(5, G.hand[0][6])) {
        printf("Assert for Function Test Passed, received value:%d\n", G.hand[0][6]);
    } else {
        printf("Assert for Function Test Failed, received value:%d\n", G.hand[0][6]);
        testState++;
     ...
     printf("Testing for discardCount, expected value:2\n");     
