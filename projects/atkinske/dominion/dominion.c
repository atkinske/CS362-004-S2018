...
int cardAdventurer(struct gameState *state, int *cardDrawn, int currentPlayer)
  {  
    int drawntreasure = 0;
	...
	*cardDrawn = state->hand[currentPlayer][state->handCount[currentPlayer]-1];//top card of hand is most recently drawn card.
	if (*cardDrawn == copper || *cardDrawn == silver || cardDrawn == gold)//TODO: Change cardDrawn == gold to *cardDrawn == gold
      ...
      while(z-1>=0){
	state->discard[currentPlayer][state->discardCount[currentPlayer]++]=temphand[z]; //TODO: Change temphand[z] to temphand [z-1] //discard all cards in play that have been drawn
	z=z-1;
      }
      return 0;  
  }	
...
{   
       //+3 Cards
      for (int i = 0; i <= 3; i++) //TODO: replace i <= 3 with i < 3
	{
	  drawCard(currentPlayer, state);
	}
  ...
  int cardCouncilRoom (int currentPlayer, struct gameState *state, int *handPos)
  {
	...
	{
	  if ( i == currentPlayer ) //TODO: change i == currentPlayer to i != currentPlayer
	    {
	      drawCard(i, state);
	    }
	}
  ...
  int cardMine(int choice1, int choice2, int currentPlayer, struct gameState *state, int *handPos)
  {
	...
	{
	  if (state->hand[currentPlayer][i] = j) //TODO: change [i] = j to [i] == j. 
	    {
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  if (card == adventurer)
  {//Adventurer
      cardAdventurer(state, &cardDrawn, currentPlayer);
      return 0;
  }
  ...
  if (card == mine)
  {
      cardMine(choice1, choice2, currentPlayer, state, &handPos);
      return 0;
  }
