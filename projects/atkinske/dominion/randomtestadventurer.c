...
int randomizeDeck(int cardQty, struct gameState *G){
    for (int i=0; i < cardQty; i++)
    {
        ...
        G.deck[player][randCard]
    }
    return player;
}

void copyGameState(struct gameState *G, struct gameState *T){
...
//random test with randomized remaining deck left, from 0 to 100 cards
    srand(time(0));
    testPlayer = randomizeDeck(&G);
