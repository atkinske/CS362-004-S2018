...
int adventurerDrawn(struct gameState *state)
{
  int currentPlayer = whoseTurn(state);
...
int councilRoomDrawn(struct gameState *state, int handPos)
{
  int i;
  int currentPlayer = whoseTurn(state);
  int nextPlayer = currentPlayer + 1;
...
int feastDrawn(struct gameState *state, int choice1) 
{
  int i;
  int x;
  int currentPlayer = whoseTurn(state);
...
int mineDrawn(struct gameState *state, int handPos, int choice1, int choice2)
{
  int i;
  int j;
//  int currentPlayer = whoseTurn(state);
//  int nextPlayer = currentPlayer + 1;
  ...
  int currentPlayer = nextPlayer + 1;
...
int smithyDrawn(struct gameState *state, int handPos)
{
  int i;
  int currentPlayer = whoseTurn(state);
  int nextPlayer = currentPlayer + 1;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
	return adventurerDrawn(state);
    case council_room:
	return councilRoomDrawn(state, handPos);
    case feast:
	return feastDrawn(state, choice1);
    case gardens:
      return -1;
    case mine:
	return mineDrawn(state, handPos, choice1, choice2);
    case remodel:
      j = state->hand[currentPlayer][choice1];  //store card we will trash
    ...
    case smithy:
	return smithyDrawn(state, handPos);
    case village:
      //+1 Card
      drawCard(currentPlayer, state);
